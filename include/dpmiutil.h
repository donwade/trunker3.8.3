...
extern long DU_alloc_desc(short desc_count_);
 ...
 * on failure */
extern int DU_alloc_dosmem(DU_WORD paragraph_count_, DU_WORD *segment_address_, DU_WORD *selector_);

/* Free memory allocated with DU_alloc_dosmem() */
extern int DU_free_dosmem(DU_WORD selector_);
...
extern DU_DWORD DU_get_dos_vect(DU_BYTE which_vect_);
...
extern void DU_set_dos_vect(DU_BYTE which_vect_, DU_WORD handler_segment_, DU_WORD handler_offset_);
...
extern void DU_set_dos_vect2(DU_BYTE which_vect_, DU_DWORD handler_);
...
extern DU_WORD DU_sim_real_int(DU_BYTE which_vect_, DU_BYTE flags_, DU_WORD copy_from_stack_, DU_REGS *regs_);

/* lock a region of memory */
extern int DU_lock_linear(void *start_, DU_DWORD size_);
