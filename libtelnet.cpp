...
#define _sendu(t, d, s) telnet_send((t), (const char *)(d), (s))
...
void telnet_free(telnet_t *telnet)
{
    ...
    //free(telnet);  //dwade (double free complaint)
}
                 ...
                 size_t size)
{
    ...
    for (l = i = 0; i != size; ++i)
    {
        ...
        if (buffer[i] == (char)TELNET_IAC)
        {
            ...
            if (i != l)
                telnet_send(telnet, buffer + l, i - l);
...
void telnet_begin_compress2(telnet_t *telnet)
{
     ...
     * instead of passing through telnet_send because telnet_send would result in
...
int telnet_vprintf(telnet_t *telnet, const char *fmt, va_list va)
{
    ...
    for (l = i = 0; i != rs; ++i)
    {
            ...
            output[i] == '\n')
        {
            ...
            if (i != l)
                telnet_send(telnet, output + l, i - l);
...
void telnet_ttype_is(telnet_t *telnet, const char *ttype)
{
    ...
    telnet_send(telnet, ttype, strlen(ttype));
