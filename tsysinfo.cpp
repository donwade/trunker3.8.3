    ...
    bandPlan('-')
{
    ...
    if (rnum >= 0)
        sprintf(fname, "%s_%.4x-cell%d.data",TRACKSCAN_PARK,sysId, rnum + 1);
    else
        sprintf(fname, "%s_%.4x-sys.data", TRACKSCAN_PARK,sysId);

    fp = fopen(fname, "rt");

    if (!fp && rnum >= 0)
    {
        sprintf(fname, "%s_%4-nbr%d.data", TRACKSCAN_PARK, sysId, rnum);
        fp = fopen(fname, "rt");
        sprintf(fname, "%s_%4x-cell%d.data", TRACKSCAN_PARK, sysId, rnum + 1);        // move to new name
    }
