...
frequencyAssignment * aid;              // points to array of assignment structures
...
frequencyAssignment * getList() const
{
	return aid;
}
...
unsigned short neighbors[64];           // for current frequency of neighbors
Filter fSite;                           // after 'n', assume stable
int naid;                               // actual number of utilized frequencies, set by FreqList
...
char sysFn[20];                         // xxxxSys.txt or xxxxRn*.txt or xxxxCn*.txt
char oldFn[20];                         // xxxxSys.txt
char idsFn[20];                         // xxxxIds.txt
char grpFn[20];                         // xxxxGrp.txt
char pageFn[20];                        // xxxxPage.txt
char patchFn[20];                       // xxxxPtch.txt
char msgFn[20];                         // xxxxMsg.txt
...
void logString(register char * cp, InfoAction ia = BOTH);
...
frequencyAssignment * findHeldCall();                   // searches frequency table to see if matches held pattern
...
void holdThis(frequencyAssignment * raid);                      // enter 'hold mode' for specified call
...
frequencyAssignment * getChannel(unsigned short fn);            // returns zero if not yet usable because of filtering
void unAssociateId(designation *);                              // internal
frequencyAssignment * expandFreqs(unsigned short newF, int thresh = 3);
